...
export default async function AcademicPage() {
  ...
  const headImage = ImageUrls.filter(p => p.name.includes("สุพัตรา"))[0]
  const headInfo = headImage.name.replace(".jpg", "").split("_")
  const memberImages = ImageUrls.filter(p => !p.name.includes("สุพัตรา"))
  const newOrderImages : GGResponeType | any = ["อัยรินทร์"].map(name => memberImages.find(i => i.name.includes(name)) )
  return (
